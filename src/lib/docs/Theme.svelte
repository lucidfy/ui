<script lang="ts">
  ...
  onMount(() => {
    if (window) {
      window.document.documentElement.setAttribute("data-theme", localStorage.theme ?? defaultTheme)
    }
  });
    ...
    <div class="grid grid-cols-1 gap-3 p-3" tabindex="0">
      {#each themes as theme}
        <div class="outline-base-content overflow-hidden rounded-lg outline outline-2 outline-offset-2" data-set-theme={theme.id} data-act-class="outline">
          <div on:click|preventDefault={() => toggleTheme(theme.id)} data-theme={theme.id} class="bg-base-100 text-base-content w-full cursor-pointer font-sans">
